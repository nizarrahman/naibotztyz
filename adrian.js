...
module.exports = conn = async (conn, m, chatUpdate, store) => {
    try {
        ...
        // Function Created By Nizar Rahman Alias Dryan ft .𝐗𝐓𝐑𝐀𝐌
...
case 'tqto': {
  ...
  *• MY PARENTS*
  *• Nizar Rahman ( Author )*
  *• Adryan Nugraha (Team NaiBotz)*
  *• Irham Khoiri Tyz (Team NaiBotz + Wibu)*
  `;
  conn.sendMessage(m.chat, {
    text: tqtonya,
    contextInfo: {
      externalAdReply: {
        ...
        sourceUrl: "https://naibotz.nizardev.site",
        mediaType: 1,
...
break
            case 'sc':
            case 'script':
               newReply(`Script Bot ${namabot} Langsung Ke Owner Aja`)
            break
        // Group Fitur
            case 'antilinkv2':
                if (!m.isGroup) return newReply(mess.group)
...
( Q.S ${res.result.data.surah.name.transliteration.id} : ${res.result.data.number.inSurah} )`
		...
		case 'brainly':{
            ...
            case 'attp':
                ...
                newReply(`🚩 5 Limit Used`)
                try {
                if (args.length == 0) return newReply(`Example: ${prefix + command} Nizar Rahman`)
            ...
            let media = await conn.downloadMediaMessage(qmsg)
            let audio = await toAudio(media, 'mp4')
            conn.sendMessage(m.chat, {document: audio, mimetype: 'audio/mpeg', fileName: `Convert By Nizar Rahman.mp3`}, { quoted : m })
	        ...
	        case 'carbon':
	            ...
	            newReply(mess.wait)
	            conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/carbon?apikey=${apikey}&code=${q}&language=nodejs`}, caption: `Created By Nizar Rahman\n\n\nCode:\n\n${q}`}, {quoted: m})
	        break
		...
		case 'freefire':
			...
			if (args.length == 0) return newReply(`Example: ${prefix + command} Nizar Rahman `)
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/ephoto1/${command}?apikey=${apikey}&text=${args}`}, caption: `Created By Nizar Rahman\n\n Type: ${command}\n\nText: ${args}`}, { quoted: m })
			break
		...
		case 'carvedwood':
			...
			if (args.length == 0) return newReply(`Example: ${prefix + command} Nizar Rahman`)
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/photooxy1/${command}?apikey=${apikey}&text=${args}`}, caption: `Created By Nizar Rahman\n\n Type: ${command}\n\nText: ${args}`}, { quoted: m})
			break
    ...
    // Text Prome
		...
		case 'wallgravity':
			...
			if (args.length == 0) return newReply(`Example: ${prefix + command} Nizar Rahman`)
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/textprome2/${command}?apikey=${apikey}&text1=${args[0]}&text2=${args[1]}`}, caption: `Created By Nizar Rahman\n\n Type: ${command}\n\nText: ${args}`}, { quoted: m })
			break
		...
		case 'thunder':
			...
			if (args.length == 0) return newReply(`Example: ${prefix + command} Nizar Rahman`)
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/textprome/${command}?apikey=${apikey}&text=${args}` }, caption: `Created By Nizar Rahman\n\n Type: ${command}\n\nText: ${args}`}, { quoted: m })
			break
